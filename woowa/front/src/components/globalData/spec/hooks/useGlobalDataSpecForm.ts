...
import useGlobalDataReqCode from '@components/globalData/req/hooks/useGlobalDataReqCode';
import {
    GlobalDataSpecFormProps,
    GlobalDataSpecParams,
    initGlobalDataSpecParams,
} from '@models/model/GlobalDataSpecModel';
import GlobalDataSpec from '@pages/user/global-data/spec';

const useGlobalDataSpecForm = ({ t, type }: Pick<GlobalDataSpecFormProps, 't' | 'type'>) => {
    const { page, params, setParams, updateParams } = usePage<GlobalDataSpecParams>(initGlobalDataSpecParams);
    ...
    // 목록 이동
    const goToList = () => {
        navigate(`${contextPath}/req/global-data/spec`);
    };
        ...
        if (isError || response?.successOrNot === 'N') {
            ...
            if (response.data) {
                ...
                if (reqContent) {
                    ...
                    newData = { zeppelinStatus: '', ...reqContent, ...reqData };
                }
...
export default useGlobalDataSpecForm;
