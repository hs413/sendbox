...
import PubCmnComments from '@/pages/publish/components/CmnComments';
...
const Main = () => {
    ...
    // 퍼블샘플 : 댓글layer 
    const [isColDivide, setColDivide] = useState(false); // <--- [isColDivide] 상태 유지
    function handleCmtOpen () {
        setColDivide(true);
    }
    function handleCmtClose () {
        setColDivide(false);
    }
    ...
    return (
        <>
            ...
            {/* 퍼블샘플 : 댓글layer : 플로팅댓글 열렸을때 .on 추가, 닫혔을때 제거 */}
            <div className={`colDivide ${isColDivide ? 'on':''} `}>
                <div className="colLeft">
                    <button style={{backgroundColor:'black',color:'#fff', width:'100%', padding:'24px'}}  onClick={handleCmtOpen}>플로팅 댓글 오픈</button>
                ...
                <div className="colRight">
                    <PubCmnComments onClose={handleCmtClose} />
                </div>
            </div>
