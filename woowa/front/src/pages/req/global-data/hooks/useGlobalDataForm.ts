...
// model
import { DP_REQ_STATUS, PageType, REQ_PAGE_MODE, ToastOption } from '@models/common/Constants';
...
const useGlobalDataForm = ({ t, type }: Pick<GlobalDataFormProps, 't' | 'type'>) => {
    ...
    const handleChangeParams = (key: string, value: any) => {
        // setValue(key as keyof GlobalDataParams, value);
    ...
    useEffect(() => {
        if (isError || response?.successOrNot === 'N') {
            ...
            if (response.data) {
                ...
                console.log('req comnent ~~', reqContent);
                if (reqContent) {
                //     if (type === REQ_PAGE_MODE.EDIT) {
                //         reqData['reqId'] = reqId;
                //     }
                //     reqData['grantStartDt'] = startDt;
                //     reqData['grantEndDt'] = endDt;
                //     reqData['realGrantStartDt'] = startDt;
                //     reqData['realGrantEndDt'] = endDt;
                //
                //     newData = { ...reqContent, ...reqData };
                //
                //     // 들어갈 데이터만큼 CurLen를 넣어준다.
                //     setReqCmtCurLen(safeLength(reqContent.reqCmt));
                // }
                //
                // if (reqContent.targetData && reqContent.targetData.length > 0) {
                //     setCheckedDataItem(reqContent.targetData);
                //
                //     const data = schemaDeduplication(reqContent.targetData);
                //     setCheckedDataItem4View(data);
                // }
                // if (reqContent.targetUser && reqContent.targetUser.length > 0) {
                //     setSelectedUserData(reqContent.targetUser);
                // }
                // if (reqContent.targetSystem && reqContent.targetSystem.length > 0) {
                //     setSystemIdList(reqContent.targetSystem);
                }
    ...
    useEffect(() => {
        if (startRequest || startTempSave) {
            const run = async () => {
                ...
                contentObj.status = startRequest ? DP_REQ_STATUS.READY : DP_REQ_STATUS.TEMP_SAVE;
                ...
                //     // 권한 적용 대상자
                //     if (!params.targetUser || params.targetUser.length == 0) {
                //         showAlert(t('schema:message.confirm.targetUser'));
                //         setStartRequest(false);
                //         return;
                //     }
                }
    ...
    return {
        params,
        errors,
        updateParams: handleChangeParams,
        formRefs,
    };
};
