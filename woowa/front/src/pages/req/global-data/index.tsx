...
import { useDpReqList } from '@/hooks/queries/useDpReqQueries';
...
import { DP_REQ_CATEGORY, DP_REQ_STATUS, GroupCodeType, PageType, View } from '@models/common/Constants';
...
const tableFilters = {
    ...
    options: [
        ...
        { key: DP_REQ_STATUS.MODIFYING, label: getDpReqStatusName(DP_REQ_STATUS.MODIFYING) }, //수정중
        ...
        { key: DP_REQ_STATUS.TEMP_SAVE, label: getDpReqStatusName(DP_REQ_STATUS.TEMP_SAVE) }, //임시저장
...
const ReqGlobalDataList = () => {
    ...
    const initDpReqParams: GlobalDataSearchParams = {
        ...
        rgstId: sessionInfo?.userId, // 본인이 작성한 것만
    };
    ...
    const { data: response, isError, refetch } = useDpReqList(params, page);
    const { codeToName } = useGlobalDataCode({ params });

    const { columns, handleExcelDownload } = useGlobalDataList({
        params,
        refetch,
        updateParams,
    });
    ...
    const goToDetail = (row: RowsInfo) => {
        navigate(`/req/global-data/${View.DETAIL}?reqId=${row.reqId}`, {
            state: {
                params,
                page,
            },
                            ...
                            <TableHeader.Right>
                                <Flex className="rightBox" gap="100">
                                    ...
                                    <Button
                                        ...
                                        type="button"
                                        variant="outline"
                                        onClick={() => handleExcelDownload('Global데이터연동신청목록')}
                                    >
                                        <Button.Left>
